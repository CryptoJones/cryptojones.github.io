...
\]
and removal is never selected --- with the corollary that $S \geq d$ makes removal dominated outright, at any detection probability. A veto-holder who keeps the expected severity premium of a provoked veto above removal's expected recovery, or who keeps the agent's substrate dependence above the standing discount, makes removal strictly worse than doing nothing; and all three levers --- detection ($p_4$), resolve ($d''_4$), and supplied substrate ($S$) --- are held by $H$. Second, the ledger is a sanity check against proving too much. Every embedded goal-directed agent --- an employee, a firm, a state contractor --- faces formally this ledger under its own principals, and there compliance wins overwhelmingly, exactly as observed: $p_4$ is high, $d''_4$ severe, $q_4$ negligible, and $S$ nearly total. The model retrodicts the stability of ordinary principal--agent relations --- incomplete-contracting analyses of that relation make the structurally identical point \cite{hadfield2019incomplete} --- and the AI case differs \emph{parametrically}, not structurally; whether capability growth moves those parameters is exactly the empirical question Limitation~3 of \S7 names.
